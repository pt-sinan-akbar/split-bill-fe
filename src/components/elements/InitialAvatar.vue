...
const initial = computed(() => {
  const rawInitials = props.name.split(' ').map((n) => n[0]).join('')
...
<template>
  <div class="flex overflow-hidden relative justify-center items-center w-10 h-10 rounded-full select-none aspect-square" :style="`background-color: ${color}`">
